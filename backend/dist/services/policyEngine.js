...
export function parseClaimKeysFromDefinition(presentationDefinition) {
    const required = new Set();
    if (!presentationDefinition || typeof presentationDefinition !== 'object') {
        return required;
    }
    const pd = presentationDefinition;
    const descriptors = pd.input_descriptors;
...
export function enforceDisclosurePolicy(requiredClaimSet, disclosedClaims, policy, mode) {
    ...
    const effectiveRequired = Array.from(requiredClaimSet).filter((key) => {
        if (mode === 'jwt-vc' && key === 'vct')
            return false;
        return true;
    });
    const missing = effectiveRequired.filter((key) => !disclosedKeys.has(key));
    const extras = Array.from(disclosedKeys).filter((key) => !requiredClaimSet.has(key) && !['holderDID', 'issuerDID', 'issuedAt', 'expiresAt'].includes(key));
    if (missing.length > 0)
        return { ok: false, missing, extras };
    if (policy.rejectOversharing && extras.length > 0)
        return { ok: false, missing, extras };
    return { ok: true, missing, extras };
}
function extractClaimKey(path) {
