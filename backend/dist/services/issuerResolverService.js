import { getTrustedIssuers, getIssuerByDid, verifyIssuer, } from '../config/trustedIssuers.js';
export const issuerResolverService = {
    /**
     * Check if issuer server is reachable
     */
    async checkServerHealth(issuerEndpoint) {
        try {
     ...
     */
    async fetchDIDDocument(issuerEndpoint, did) {
        try {
     ...
     */
    async fetchPublicKeyDirect(issuerEndpoint) {
        try {
            const endpoints = [
     ...
     */
    async getAllIssuersStatus() {
        return await getTrustedIssuers();
    },
    /**
     * Verify credential signature with issuer public key
     */
    async verifyCredentialSignature(credentialDid, signature) {
...
//# sourceMappingURL=issuerResolverService.js.map