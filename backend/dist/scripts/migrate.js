...
async function migrate() {
    ...
    `);
        ...
        await query(`CREATE INDEX IF NOT EXISTS idx_verification_sessions_expires ON verification_sessions(expires_at);`);
        await query(`CREATE UNIQUE INDEX IF NOT EXISTS idx_verification_sessions_state ON verification_sessions(state) WHERE state IS NOT NULL;`);
        ...
        if (existingUser.rows.length === 0) {
            ...
            await query(`INSERT INTO users (id, email, password, name, fasikes_name, role) VALUES ($1, $2, $3, $4, $5, $6)`, [uuidv4(), 'admin@rumahsakit.com', hashedPassword, 'Admin RS Medika', 'Rumah Sakit Medika', 'admin']);
        ...
        if (existingIssuer.rows.length === 0) {
            await query(`INSERT INTO trusted_issuers (did, name, endpoint, verified) VALUES ($1, $2, $3, $4)`, ['did:web:202.155.132.71%3A3001:issuer', 'BPJS Healthcare & Central Authority', 'http://202.155.132.71:3001', false]);
