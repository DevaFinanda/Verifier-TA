...
import { apiLimiter, hppProtection, sanitizeInputs, xssHeaders, validateRequestSize, securityLogger, contentSecurityPolicy, } from './middleware/security.js';
// Create Express app
const app = express();
...
app.use((req, res, next) => {
    ...
    res.json = ((body) => {
        ...
        const nestedReasonCodes = payload?.data?.reasonCodes;
        if (Array.isArray(topReasonCodes)) {
            res.locals.reasonCodes = topReasonCodes;
        }
        else if (Array.isArray(nestedReasonCodes)) {
            res.locals.reasonCodes = nestedReasonCodes;
        }
        return originalJson(body);
    });
    res.on('finish', () => {
        const durationMs = Date.now() - start;
        const localReasonCodes = res.locals.reasonCodes;
 ...
 */
export function mountCredoApp(credoApp) {
    app.use((req, res, next) => {
        if (!req.path.startsWith('/oid4vp')) {
            return next();
        }
        ...
        const isAuthorizationResponse = /^\/oid4vp\/[\w-]+\/authorization-response$/.test(req.path)
        ...
        if (req.method === 'POST' && isAuthorizationResponse) {
            oid4vpService.inspectIncomingAuthorizationResponse(req.body)
                ...
                if (!inspection.ok) {
                    return sendError(res, 400, 'BAD_REQUEST', 'invalid_request', {
                        reason: {
                            step: 'pre_validation',
                ...
                try {
                    const verified = await verifyPresentation(vpToken, session.nonce, presentationSubmission, BPJS_PRESENTATION_DEFINITION);
                    await oid4vpService.markSuccess(session.id, verified.claims, verified.claims.holderDID ?? null, vpToken, {
                    ...
                    return sendSuccess(res, 'authorization_response_accepted', {
                        status: 'SUCCESS',
