...
function addCredentialWarnings(
  ...
  credentialSubject: Record<string, unknown> | null | undefined,
  warnings: string[],
): void {
  ...
  if (!credentialSubject) {
    ...
    if (!nik) warnings.push('nik missing');
    if (typeof nik === 'string' && nik.length !== 16) warnings.push('nik length is not 16');
    if (!noBPJS) warnings.push('noBPJS missing');
  }
...
async function verifyJWTSignature(
  ...
  context: { step: 'VC' | 'VP' | 'KB'; kid?: string },
  ...
  console.log(`SIGNATURE ${context.step}:`, {
    ...
    keyExport: (() => {
      try {
        return publicKey.export({ format: 'jwk' as const });
      } catch {
        return null;
      }
    })(),
...
function detectFormat(vpToken: string, hintedFormat?: string): 'sd-jwt' | 'jwt_vc' {
  if (vpToken.includes('~')) return 'sd-jwt';
...
function normalizeFormatHint(format?: string): string | null {
  if (!format) return null;
  return format.trim().toLowerCase();
}

function getSubmissionFormatHint(presentationSubmission?: unknown): string | undefined {
  const normalizedSubmission = normalizePresentationSubmission(presentationSubmission);
  if (!normalizedSubmission || typeof normalizedSubmission !== 'object') return undefined;
  const descriptorMap = (normalizedSubmission as Record<string, unknown>).descriptor_map;
  if (!Array.isArray(descriptorMap) || descriptorMap.length === 0) return undefined;
  const first = descriptorMap[0];
  if (!first || typeof first !== 'object') return undefined;
  const format = (first as Record<string, unknown>).format;
...
async function verifyJwtVcVP(
  ...
  let holderPublicKey: crypto.KeyObject | null = null;
  ...
  const vpPayload = verifyPresentationSignature && holderPublicKey
    ...
    : (rawDecoded.payload as Record<string, unknown>);
  checks.push({
    name: 'signature_vp',
    passed: true,
  ...
  let issuerPublicKey: crypto.KeyObject | null = null;
  ...
  if (!issuerDID) credentialWarnings.push('issuer DID missing');
  if (!subject?.id) credentialWarnings.push('credentialSubject.id missing');
  if (!vcPayload.nbf && !vcPayload.iat) credentialWarnings.push('issued-at timestamp missing');
  if (!vcPayload.exp) credentialWarnings.push('expiration timestamp missing');
  if (!typeArr?.length) credentialWarnings.push('credential type missing');
  if (!credentialStatus?.id) credentialWarnings.push('credentialStatus.id missing');
...
async function verifySDJwtVP(
  ...
  const credentialWarnings: string[] = [];
  if (!issuerPayload.iss) credentialWarnings.push('issuer DID missing');
  if (!holderDID) credentialWarnings.push('holder DID missing');
  if (!issuerPayload.nbf && !issuerPayload.iat) credentialWarnings.push('issued-at timestamp missing');
  if (!issuerPayload.exp) credentialWarnings.push('expiration timestamp missing');
  if (!hasStatusField) credentialWarnings.push('credential status field missing');
