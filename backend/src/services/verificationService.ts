...
export const verificationService = {
  ...
  async createRequest(
    ...
    // Save to database
    await query(
      `INSERT INTO verification_requests (id, request_id, verifier_id, attributes, nonce, timestamp, status, qr_data)
       VALUES ($1, $2, $3, $4, $5, $6, $7, $8)`,
      [request.id, request.requestId, request.verifierId, request.attributes, request.nonce, request.timestamp, request.status, request.qrData]
    );
   ...
   */
  async getRequest(requestId: string): Promise<VerificationRequest | undefined> {
    const result = await query(
      'SELECT id, request_id, verifier_id, attributes, nonce, timestamp, status, qr_data FROM verification_requests WHERE request_id = $1',
      [requestId]
    );
   ...
   */
  async simulateScan(requestId: string): Promise<{ logs: string[]; request: VerificationRequest }> {
   ...
   */
  async verifyCredentialSignature(issuerDid: string, credential: any): Promise<{
    valid: boolean;
  ...
  async completeVerification(
    requestId: string,
    tujuanPoli: string = 'Poli Umum',
    credential?: any  // Real VC from holder (optional - if not provided, uses DEMO mode)
    ...
    if (!issuerDid) {
      ...
      await query(
        `INSERT INTO verification_results (id, request_id, status, patient_data, error_message, verified_at)
         VALUES ($1, $2, $3, $4, $5, $6)`,
        [result.id, requestId, result.status, null, result.errorMessage, result.verifiedAt]
      );
    ...
    await query(
      `INSERT INTO verification_results (id, request_id, status, patient_data, error_message, verified_at)
       VALUES ($1, $2, $3, $4, $5, $6)`,
      [result.id, requestId, result.status, JSON.stringify(patientData), null, result.verifiedAt]
    );
    ...
    await query(
      ...
      [log.id, log.requestId, log.waktu, log.namaPasien, log.tujuanPoli, log.status, log.verifierId]
    );
  ...
  async getHistory(
    ...
    page: number = 1,
    limit: number = 10
  ): Promise<{ logs: VerificationLog[]; total: number; page: number; totalPages: number }> {
    ...
    const logs: VerificationLog[] = dataResult.rows.map((row: any) => ({
      id: row.id,
      requestId: row.request_id,
      waktu: row.waktu,
   ...
   */
  async getLogByRequestId(requestId: string): Promise<VerificationLog | undefined> {
    const result = await query(
      'SELECT id, request_id, waktu, nama_pasien, tujuan_poli, status, verifier_id FROM verification_logs WHERE request_id = $1',
      [requestId]
    );
    if (result.rows.length === 0) return undefined;
    const row = result.rows[0];
