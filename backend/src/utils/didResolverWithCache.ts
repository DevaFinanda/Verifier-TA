...
import { didCache, type DIDDocument } from './didCache.js';
 ...
 */
async function fetchDIDDocumentFromNetwork(did: string): Promise<DIDDocument | null> {
  ...
  if (did.startsWith('did:web:')) {
    try {
      ...
      console.warn(`  ⚠️  Direct did:web fetch gagal: ${(err as Error).message}`);
    }
  }
  ...
  try {
    ...
    console.warn(`  ⚠️  Trust anchor fetch gagal: ${(err as Error).message}`);
  }
 ...
 */
export async function resolveDidWithCache(did: string): Promise<DIDDocument> {
  await didCache.init();
  ...
  if (stale) {
    ...
    console.warn(
      `  🔄 DID fallback from cache: ${did} ` +
      `(stale ${ageHr.toFixed(1)} jam, server tidak tersedia)`,
    );
    if (networkError) {
      console.warn(`     Network error: ${(networkError as Error).message}`);
    }
