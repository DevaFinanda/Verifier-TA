...
interface CredoVerifierState {
  agent: Agent<any> | null;
  verifierRecord: any;
  verifierDidUrl: string | null;
}

function getState(): CredoVerifierState {
...
export async function initCredoVerifier(): Promise<{
  credoApp: Express;
}> {
  ...
  const agent = new Agent({
    ...
    config: {
      ...
      logger: new ConsoleLogger(
        config.nodeEnv === 'development' ? LogLevel.warn : LogLevel.error
      ),
    },
    modules: {
  ...
  const verifierApi = (agent as any).modules.openId4Vc.verifier;
  ...
  agent.events.on<OpenId4VcVerificationSessionStateChangedEvent>(
    ...
    async (event: any) => {
      ...
      console.log(
        `  📡 Verification session ${session.id}: ${prevState} → ${session.state}`
      );
  ...
  const credoApp = (agent as any).modules.openId4Vc.config.app as Express;
...
export function getVerifierApi() {
  ...
  return (agent as any).modules.openId4Vc.verifier;
}
...
export function getVerifierDidUrl(): string {
  const state = getState();
