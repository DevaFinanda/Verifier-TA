...
function run(): void {
  ...
  const validDescriptor = validateDescriptorMap(
    presentationDefinition,
    { descriptor_map: [{ id: 'bpjs-health-credential', path: '$.vp.verifiableCredential[0]' }] },
    vpPayload,
  );
  ...
  const stringDescriptor = validateDescriptorMap(
    presentationDefinition,
    JSON.stringify({ descriptor_map: [{ id: 'bpjs-health-credential', path: '$.verifiableCredential[0]' }] }),
    vpPayload,
  );
  assert.equal(stringDescriptor.ok, true);

  // Case 2: invalid descriptor path
  const invalidDescriptor = validateDescriptorMap(
    presentationDefinition,
    { descriptor_map: [{ id: 'bpjs-health-credential', path: '$.vp.verifiableCredential[7]' }] },
    vpPayload,
  );
  ...
  const policyResult = enforceDisclosurePolicy(
    new Set(['holderName', 'nik']),
    { holderName: 'Budi', nik: '3201', tanggalLahir: '2000-01-01' },
    {
